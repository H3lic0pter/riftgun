...
public final class ModeRadialInput {
    ...
    private static boolean closeWasDown;
    private static int closeHeldTicks;
    ...
    public static void tick() {
        ...
        boolean closeDown = keys.closeDown();
        ...
        if (suppressUntilRelease) {
            if (!cycleDown && !radialDown && !surfacePreviewDown && !closeDown) {
        ...
        if (pendingSource != null) {
            if (!sourceDown(pendingSource, keys)) {
                if (pendingSource == Source.CLOSE) cancelFromScreen();
        ...
        if (closeDown) {
            closeHeldTicks = closeWasDown ? closeHeldTicks + 1 : 1;
            if (closeHeldTicks == HOLD_TICKS) request(Source.CLOSE);
            remember(keys);
            return;
        } else if (closeWasDown) {
            if (closeHeldTicks > 0 && closeHeldTicks < HOLD_TICKS) {
                ModeRadialClientAccess.sendCloseAllRequest();
            }
            closeHeldTicks = 0;
        } else if (keys.closeClicked()) {
            // A complete tap can occur between client ticks.
            ModeRadialClientAccess.sendCloseAllRequest();
        }
    ...
    public static void openFromServer(int requestId) {
        ...
        boolean stillDown = sourceDown(pendingSource, ModeRadialClientAccess.keys());
        if (pendingSource == Source.CLOSE && !stillDown) {
            cancelFromScreen();
            return;
        }
        RadialRequestState.AcknowledgeResult result = REQUEST.acknowledge(requestId, stillDown);
    ...
    private static void request(Source source) {
        ...
        ModeRadialClientAccess.sendOpenRequest(
            requestId, source == Source.PRECISION_PREVIEW, source == Source.CLOSE);
    }

    private static boolean sourceDown(Source source, ModeRadialClientAccess.Keys keys) {
        return switch (source) {
            case CYCLE -> keys.cycleDown();
            case DEDICATED -> keys.radialDown();
            case PRECISION_PREVIEW -> keys.precisionDown();
            case CLOSE -> keys.closeDown();
        };
    }

    private static void remember(ModeRadialClientAccess.Keys keys) {
        cycleWasDown = keys.cycleDown();
        radialWasDown = keys.radialDown();
        surfacePreviewWasDown = keys.precisionDown();
        pairingOperationWasDown = keys.pairingOperationDown();
        closeWasDown = keys.closeDown();
    }

    private static void reset(ModeRadialClientAccess.Keys keys) {
        pendingSource = null;
        pendingPrecisionRequest = null;
        REQUEST.cancel();
        cycleHeldTicks = 0;
    ...
    private enum Source { CYCLE, DEDICATED, PRECISION_PREVIEW, CLOSE }
