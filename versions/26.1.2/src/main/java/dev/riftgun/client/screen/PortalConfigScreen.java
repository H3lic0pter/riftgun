...
public final class PortalConfigScreen extends Screen {
    ...
    private void renderPlacementTooltips(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (session.page() == PortalConfigPage.SETTINGS) {
            ...
            if (shotAnimationSettingsButton != null && shotAnimationSettingsButton.isHovered()) {
                graphics.setComponentTooltipForNextFrame(font, List.of(Component.translatable("screen.riftgun.shot_animation_settings")), mouseX, mouseY);
            }
        }
        if (session.page() == PortalConfigPage.GUN_SETTINGS) {
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (event.key() == 256 && (session.page() == PortalConfigPage.SHOT_ANIMATION_SETTINGS
