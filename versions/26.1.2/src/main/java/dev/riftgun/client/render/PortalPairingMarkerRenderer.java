...
final class PortalPairingMarkerRenderer {
    ...
    static void submit(PortalVisualRenderContext context, PortalEntity portal, int color) {
        PortalPairingEndpoint endpoint = portal.pairingEndpoint();
        if (endpoint == PortalPairingEndpoint.NONE
            || context.surfaceRenderPath() == PortalSurfaceRenderPath.SKIP_SURFACE) return;
        float scale = Mth.sin(portal.visualProgress(context.partialTick()) * Mth.HALF_PI);
        ...
        context.submit(PortalRenderTypes.border(), (pose, vertices) -> {
            switch (endpoint) {
