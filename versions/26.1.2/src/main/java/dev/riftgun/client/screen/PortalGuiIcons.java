...
final class PortalGuiIcons {
    ...
    static void drawColorIcon(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
        GuiSprite.draw(graphics, PortalGuiSprites.COLOR,
    ...
    static void drawBucketIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, color == PortalTheme.TEXT_MUTED
    ...
    static void drawPlacementModeIcon(GuiGraphicsExtractor graphics, int spriteX, int spriteY,
                                      PortalPlacementMode mode) {
        GuiSprite sprite = switch (mode) {
        ...
        GuiSprite.draw(graphics, sprite, spriteX, spriteY);
    }

    /** Accepts the compact button origin and preserves the reviewed 45bcf94 artwork position. */
    static void drawPlacementModeButtonIcon(GuiGraphicsExtractor graphics, int buttonX, int buttonY,
                                            PortalPlacementMode mode) {
    ...
    static void drawGunSettingsIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.CONFIGURE_GUN, x - 3, y - 2);
    }

    static void drawModuleBayIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.MODULE_BAY, x - 2, y - 2);
    }

    static void drawPortalCloseIcon(GuiGraphicsExtractor graphics, int x, int y) {
        GuiSprite.draw(graphics, PortalGuiSprites.PORTAL_CLOSE, x - 2, y - 2);
    }

    /** Accepts button bounds and centers the 16 x 16 navigation sprite. */
    static void drawDimensionalTraversalIcon(GuiGraphicsExtractor graphics, int buttonX, int buttonY,
                                             int buttonWidth, int buttonHeight) {
        GuiSprite.draw(graphics, PortalGuiSprites.DIMENSIONAL_TRAVERSAL,
            buttonX + (buttonWidth - ICON_SIZE) / 2,
            buttonY + (buttonHeight - ICON_SIZE) / 2);
    }

    static void drawSmartDistanceIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.SMART_DISTANCE, x - 2, y - 2);
    }

    static void drawPortalDurationIcon(GuiGraphicsExtractor graphics, int x, int y) {
        GuiSprite.draw(graphics, PortalGuiSprites.PORTAL_DURATION, x - 2, y - 2);
    }

    static void drawApertureIcon(GuiGraphicsExtractor graphics, int x, int y, boolean enabled) {
        GuiSprite.draw(graphics, enabled
    ...
    static void drawSurfaceRangeIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.SURFACE_RANGE, x - 3, y - 2);
    }

    static void drawEntityAccessIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.ENTITY_ACCESS, x - 2, y - 2);
    }
    ...
    static void drawPlayerRefreshIcon(GuiGraphicsExtractor graphics, int x, int y) {
        GuiSprite.draw(graphics, PortalGuiSprites.PLAYER_REFRESH, x - 4, y - 4);
    }
    ...
    static void drawCompactBackButtonIcon(GuiGraphicsExtractor graphics, int buttonX, int buttonY, int buttonWidth) {
    ...
    static void drawSwirlIcon(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.SWIRL, x - 4, y - 4);
    }

    static void drawDisclosure(GuiGraphicsExtractor graphics, int x, int y, boolean expanded) {
        GuiSprite.draw(graphics, expanded
    ...
    static void drawDragHandle(GuiGraphicsExtractor graphics, int x, int y) {
        GuiSprite.draw(graphics, PortalGuiSprites.DRAG_HANDLE, x - 5, y - 4);
    }
    ...
    static void drawStar(GuiGraphicsExtractor graphics, int x, int y, boolean filled) {
        GuiSprite.draw(graphics, filled ? PortalGuiSprites.STAR_ON : PortalGuiSprites.STAR_OFF,
            x - 4, y - 4);
    }

    static void drawCross(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.DELETE, x - 4, y - 4);
    }

    static void drawPencil(GuiGraphicsExtractor graphics, int x, int y, int color) {
        GuiSprite.draw(graphics, PortalGuiSprites.EDIT,
