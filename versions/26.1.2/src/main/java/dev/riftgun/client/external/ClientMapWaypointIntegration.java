...
public final class ClientMapWaypointIntegration {
    ...
    public static boolean refreshJourneyMapIfDirty(Set<String> dimensions, int limit) { if (!journeyMapDirty) return false; refreshSource(ExternalDestinationSource.JOURNEYMAP, dimensions, limit); journeyMapDirty = false; return true; }
    ...
    private static void installAdapter(ExternalDestinationSource source) { if (!installed(source)) return; try { ClientExternalDestinationAdapter adapter = switch (source) { case JOURNEYMAP -> new JourneyMapExternalDestinationAdapter(); case XAERO_MINIMAP -> new XaeroExternalDestinationAdapter(); }; ADAPTERS.put(source, adapter); } catch (LinkageError | RuntimeException exception) { disable(source, exception); } }
    private static void disable(ExternalDestinationSource source, Throwable exception) { ADAPTERS.remove(source); CATALOG.replace(new ExternalDestinationReadResult(source, ExternalDestinationReadResult.Status.INCOMPATIBLE, installedVersion(source), exception.getClass().getSimpleName(), List.of()), Set.of(), 1); if (WARNED.add(source)) LOGGER.warn("RiftGun disabled {} waypoint integration for version {}", source.displayName(), installedVersion(source), exception); }
