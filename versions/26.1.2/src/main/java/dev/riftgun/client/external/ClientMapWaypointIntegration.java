...
public final class ClientMapWaypointIntegration {
    ...
    public static void refresh(Set<String> dimensions, int limit) { initialize(); for (ExternalDestinationSource source : ExternalDestinationSource.values()) refreshSource(source, dimensions, limit); journeyMapDirty = false; }
    public static void refreshJourneyMapIfDirty(Set<String> dimensions, int limit) { if (!journeyMapDirty) return; refreshSource(ExternalDestinationSource.JOURNEYMAP, dimensions, limit); journeyMapDirty = false; }
    ...
    public static void select(ExternalDestination destination) { selected = new ExternalDestinationSelection(destination.source(), destination.stableId(), destination.name(), destination.dimensionId(), destination.x(), destination.y(), destination.z()); }
    ...
    private static void initialize() { if (initialized) return; initialized = true; if (installed(ExternalDestinationSource.JOURNEYMAP)) ADAPTERS.put(ExternalDestinationSource.JOURNEYMAP, new JourneyMapExternalDestinationAdapter()); if (installed(ExternalDestinationSource.XAERO_MINIMAP)) ADAPTERS.put(ExternalDestinationSource.XAERO_MINIMAP, new XaeroExternalDestinationAdapter()); }
    ...
    private static void refreshSource(ExternalDestinationSource source, Set<String> dimensions, int limit) { ClientExternalDestinationAdapter adapter = ADAPTERS.get(source); if (adapter == null) return; if (!enabled(source)) { CATALOG.clear(source); if (selected != null && selected.source() == source) selected = null; return; } ExternalDestinationReadResult result; try { result = adapter.read(installedVersion(source)); } catch (LinkageError | RuntimeException exception) { result = new ExternalDestinationReadResult(source, ExternalDestinationReadResult.Status.INCOMPATIBLE, installedVersion(source), exception.getClass().getSimpleName(), List.of()); if (WARNED.add(source)) LOGGER.warn("RiftGun disabled {} waypoint integration for version {}", source.displayName(), installedVersion(source), exception); } CATALOG.replace(result, dimensions, limit); }
