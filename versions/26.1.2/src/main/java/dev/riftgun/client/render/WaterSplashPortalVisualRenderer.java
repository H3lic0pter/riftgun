...
final class WaterSplashPortalVisualRenderer implements PortalVisualRenderer {
    ...
    public void submit(PortalVisualRenderContext context) {
        ...
        int light = client.level == null ? 0 : LevelRenderer.getLightCoords(client.level, BlockPos.containing(portal.placement().center()));
        double cosine = Math.cos(rotation);
        double sine = Math.sin(rotation);
        // Shader packs use this same vanilla-material fallback until a native water path is verified.
        var material = RenderTypes.entityTranslucent(TextureAtlas.LOCATION_BLOCKS, false);
        context.submit(material, (pose, vertices) -> draw(pose, vertices, basis,
