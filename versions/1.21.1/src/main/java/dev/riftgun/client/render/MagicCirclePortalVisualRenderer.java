...
final class MagicCirclePortalVisualRenderer implements PortalVisualRenderer {
    ...
    private static void drawLayer(PortalVisualRenderContext context, PortalRenderBasis basis,
        ...
        if (context.surfaceRenderPath() == PortalSurfaceRenderPath.VANILLA_FALLBACK) {
            ...
            draw(context.poseStack().last().pose(), context.buffers().getBuffer(ShaderMaterials.BODY[layer]), basis,
                width, height, color, alpha, cosine, sine, false, face);
            return;
        }
        draw(context.poseStack().last().pose(), context.buffers().getBuffer(Materials.BODY[layer]), basis,
            width, height, color, alpha, cosine, sine, false, 0);
        draw(context.poseStack().last().pose(), context.buffers().getBuffer(Materials.GLOW[layer]), basis,
            width, height, color, alpha, cosine, sine, true, 0);
    }
