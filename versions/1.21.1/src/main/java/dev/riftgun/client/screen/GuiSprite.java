...
record GuiSprite(ResourceLocation id, int size) {
    GuiSprite(String path, int size) {
        this(ResourceLocation.fromNamespaceAndPath(RiftConstants.MOD_ID, path), size);
    }

    /** Accepts the sprite canvas top-left. One native atlas draw, with no per-draw allocation. */
    static void draw(GuiGraphics graphics, GuiSprite sprite, int x, int y) {
        graphics.blitSprite(sprite.id, x, y, sprite.size, sprite.size);
    }

    /** Accepts button bounds; a 16 px sprite in a 26 px button has a 5 px inset. */
    static void drawCentered(GuiGraphics graphics, GuiSprite sprite,
    ...
    static void drawAtCenter(GuiGraphics graphics, GuiSprite sprite, int centerX, int centerY) {
