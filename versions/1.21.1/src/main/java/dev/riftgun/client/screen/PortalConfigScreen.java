...
public final class PortalConfigScreen extends Screen {
    ...
    private void initModal() {
        ...
        if (session.page() == PortalConfigPage.CREATE_COORDINATE || session.page() == PortalConfigPage.EDIT_DESTINATION) {
            ...
            int maximum = Math.max(1, PortalClientState.gun().maximumSurfaceRange());
            addRenderableWidget(new GunDistanceSlider(x + 18, gunSettingControlY, fieldWidth, 18,
        ...
        if (playerTargets.selectedId() != null) {
            ...
            if (entry == null) {
                ...
                if (mouseX >= x && mouseX < right - 6
                    ...
                    graphics.renderTooltip(font, Component.literal(dimension), mouseX, mouseY);
                }
            ...
            String dimension = displayDimension(destination.dimension().location().toString());
            int dimensionY = y;
            y = detailField(graphics, "screen.riftgun.dimension", dimension, x, y, textWidth);
            if (mouseX >= x && mouseX < right - 6
                && mouseY >= dimensionY + 9 && mouseY < dimensionY + 23) {
    ...
    private void renderModal(GuiGraphics graphics) {
        ...
        graphics.drawString(font, Component.translatable(session.page().titleKey()), box.x() + 18, box.y() + 13,
            PortalTheme.TEXT, false);
    ...
    private void renderVisualOptionsChrome(GuiGraphics graphics, Box box) {
        PortalVisualOptions options = PortalVisualPreferences.selected().options();
        if (options.isEmpty()) return;
        int top = visualOptionsTop(box);
        int bottom = visualOptionsBottom(box);
    ...
    private void renderPlacementTooltips(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        if (session.page() == PortalConfigPage.NONE && placementModeButton != null) {
            ...
            if (dimensionalTraversalButton != null && dimensionalTraversalButton.isHovered()) {
                graphics.renderTooltip(font,
                    Component.translatable("screen.riftgun.dimensional_navigation"), mouseX, mouseY);
            }
            if (motionPredictionButton != null) {
    ...
    private void renderSelectorDropdown(GuiGraphics graphics, int mouseX, int mouseY) {
        ...
        for (int index = 0; index < selectorChoices.size(); index++) {
            ...
            graphics.drawString(font, trim(choice.label().getString(), box.width() - 12), box.x() + 6, rowY + 5,
    ...
    private void drawGunSettingDescription(GuiGraphics graphics, Box box) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (selectorDropdownOpen()) {
            if (button == 0) clickSelectorDropdown(mouseX, mouseY);
            closeSelectorDropdown();
            return true;
        }
        ...
        if (session.page() == PortalConfigPage.SHOT_ANIMATION_SETTINGS && shotAnimationSelector != null
            && (button == 0 || button == 1) && mouseX >= shotAnimationSelector.getX()
            && mouseX < shotAnimationSelector.getX() + shotAnimationSelector.getWidth()
            && mouseY >= shotAnimationSelector.getY() && mouseY < shotAnimationSelector.getY() + shotAnimationSelector.getHeight()) {
            GunShotAnimation before = dev.riftgun.client.appearance.SkinRecommendations.current().animation();
            shiftShotAnimation(button == 0 ? 1 : -1);
        ...
        if (session.page() == PortalConfigPage.SOUND_SETTINGS && (button == 0 || button == 1)) {
        ...
        if (session.page() != PortalConfigPage.NONE) return super.mouseClicked(mouseX, mouseY, button);
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (selectorDropdownOpen()) return selectorDropdownKeyPressed(keyCode);
        ...
        if (session.page() == PortalConfigPage.SHOT_ANIMATION_SETTINGS && shotAnimationSelector != null && shotAnimationSelector.isFocused()
            && (keyCode == 263 || keyCode == 262)) {
            shiftShotAnimation(keyCode == 263 ? -1 : 1);
            return true;
        }
        if (session.page() == PortalConfigPage.SOUND_SETTINGS && (keyCode == 263 || keyCode == 262)) {
            for (var entry : soundSelectors.entrySet()) {
        ...
        if (keyCode == 256 && session.page() == PortalConfigPage.SWIRL_ANIMATION_SETTINGS) {
            backToVisualSettings();
            return true;
        }
        if (keyCode == 256 && session.page().isGunSettingPage()) {
            backToGunSettings();
            return true;
        }
        if (keyCode == 256 && session.page() == PortalConfigPage.VISUAL_SETTINGS) {
            backToSettings();
            return true;
        }
        if (keyCode == 256 && session.page() == PortalConfigPage.SOUND_SETTINGS) {
            backToSettings();
            return true;
        }
        ...
        if (keyCode == 256 && session.page() != PortalConfigPage.NONE) {
            requestCloseModal();
            return true;
        }
        ...
        if (session.page() == PortalConfigPage.NONE && keyCode == 258) {
        ...
        if (session.page() == PortalConfigPage.NONE && listFocused && listKeyPressed(keyCode)) return true;
    ...
    private void submitModal() {
        switch (session.page()) {
            ...
            case RENAME_GROUP -> PortalNetworking.sendRequest(PortalAction.RENAME_GROUP, tag -> {
                if (session.target() != null) tag.putUUID("Group", session.target());
    ...
    private void sendDestinationForm(PortalAction action, boolean coordinates) {
        PortalNetworking.sendRequest(action, tag -> {
            ...
            if (action == PortalAction.EDIT_DESTINATION && session.target() != null) {
                tag.putUUID("Destination", session.target());
            }
            if (coordinates) {
